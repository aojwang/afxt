...
from utils.constants import STOCK_TICK_DATA, STOCK_DAILY
...
class TickData(object):
    ...
    def create_stock_tick_table(self):
        self.runner.execute("DROP TABLE IF EXISTS %s" % STOCK_TICK_DATA)
        dbutil.create_table(self.runner, STOCK_TICK_DATA, """
            CREATE TABLE {name}
            (
                index BIGINT,
                name TEXT,
...
def convert_tick_to_daily():
    sql = """
        INSERT INTO {stock_daily}(
          code, date, open, high, close, volume,
          price_change, p_change, ma5, ma10, ma20,
          v_ma5, v_ma10, v_ma20, turnover)
        SELECT s.code, s.date, s.open, s.high, s.price, s.volume,
          s.price - s.pre_close,
